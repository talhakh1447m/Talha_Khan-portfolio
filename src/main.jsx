...
function Assignments(){return <div><div className="assignment-top"><div className="filters assignment-filters"><button className="active">All</button>{["AI","DBMS","OS","CN","Python","Cloud","Other"].map(x=><button key={x}>{x}</button>)}</div><div className="mini-search assignment-search"><Search size={14}/><input placeholder="Search assignments..." /></div></div><div className="assignment-grid">{assignments.map(a=><article className="assignment-card" key={a.id}><div className="assignment-thumb"><img src={a.image} alt=""/></div><h3>{a.title}</h3><p>{a.type}</p><small>{a.tags.join(" • ")}</small></article>)}</div><button className="view-all">View All Assignments <ChevronRight size={15}/></button></div>}
